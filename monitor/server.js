...
const boss = {
  name: "User teste",
  telephone: process.env.USERTESTE_TELEPHONE,
};
  ...
  for (const server of servers) {
    await axios({
      ...
      .catch(() => {
        ...
        client.tts
          .enviar(server.developer.telephone, message, options)
          .then((response) => {
            console.log(
              `O desenvolvedor ${server.developer.name} já foi avisado!`
            );
    ...
    if (notification.status === "pending") {
      client.tts.buscar(notification.id).then(function (response) {
        if (response.dados.resposta === "1") {
          console.log(
            `O desenvolvedor ${notification.server.developer.name} já foi avisado e vai fazer alguma coisa!`
          );
        ...
        if (response.dados.resposta === "2") {
          console.log(
            `O desenvolvedor ${notification.server.developer.name} já foi avisado e não pode fazer nada!`
          );
